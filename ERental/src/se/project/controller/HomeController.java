...
public class HomeController implements Initializable {
	...
	void chooseBike(MouseEvent event) {
		...
		bike = iBike.getBikeFromDB(staffBike.getSelectionModel().getSelectedItem().getName(),
				staffBike.getSelectionModel().getSelectedItem().getType());
		  
		FXMLLoader loader = new FXMLLoader();
		loader.setLocation(getClass().getResource("/se/project/gui/itemDetail.fxml"));
		try {
			...
			if(order.getBike()==null) {
				bikeRent = null;
			} else bikeRent = order.getBike();  // order chua co thi bikeRENT = BIKE
			controller.setBike(bikeRent);
			//controller.setBike(order.getBike());
			controller.setCustomer(customer);
			
			// set bike info
			File file = new File("src/se/project/image/"
					+ staffBike.getSelectionModel().getSelectedItem().getName() + ".jpeg");
			Image image = new Image(file.toURI().toString());
			// add bike info to pane
			controller.getImg().setImage(image);
			controller.getName().setText(bike.getName());
			controller.getType().setText(bike.getType());
			controller.getWeight().setText(Integer.toString(bike.getWeight()));
			controller.getLicense().setText(bike.getLicense());
			controller.getManufacture().setText(bike.getManufacture());
			controller.getProducer().setText(bike.getProducer());
