...
import se.project.util.BankUtils;
import se.project.util.SendEmail;
import se.project.util.TimeUtils;
...
public class bankGateController<PayStrategy> implements Initializable {
    ...
    */
		...
		if (name.getText() == "" || cardNum.getText() == "" || bank.getText() == ""
			...
			if(order.getId()!=0) {
				Thread t2 = new Thread(new Runnable() {
					@Override
					public void run() {
					    iTransaction.saveTransaction(order.getId(), messageA.getText(), order.getBike().getDeposit());
					    
					    /* TO DO        IMPLEMENT ORDER.GETTOTAL        */
						iTransaction.updateReturn(order.getId(),order.getBike().getId(),order.getTotal(),order.getTimeFinish());  // order.get total thay cho 0
						  /* TO DO        IMPLEMENT pick store return o ORDER CONTROLLER       */
						iStore.updateStoreReturn(order.getBike().getId(),1);  // store id from return return ve store nao
	...
	public void load() {
		FXMLLoader loader = new FXMLLoader();
		loader.setLocation(getClass().getResource("/se/project/gui/payResult.fxml"));
		try {
			Parent root = loader.load();
			ResultController result = loader.getController();
			result.setOrder(order);
			result.setMsg(messageA.getText());
			result.setMoney(Integer.toString(order.getBike().getDeposit()));
