...
public class Login {
  ...
  public static boolean validate(String user,String pass) {
	  try { 
          ...
          PreparedStatement ps = con.prepareStatement("SELECT * FROM useraccount WHERE user =? AND pass =?");
          ps.setString(1, user);
          ps.setString(2, pass);
  ...
  public static void signUp(String user,String pass) {
	  try {
          ...
          Connection con = Context.getConnection();
          PreparedStatement pss = con.prepareStatement("INSERT INTO useraccount SET user = ? , pass = ?");
